...
export class ExhumacionService {
  ...
  async create(
    ...
    try {
      ...
      //Validar si ya existe una exhumación previa para esta inhumación
      const exhumacionExistente = await this.exhumacionRepository.findOne({
        where: { inhumacion_id: createDto.inhumacion_id },
      });

      if (exhumacionExistente) {
        throw new BadRequestException(
          `Ya existe una exhumación registrada para esta inhumación (${exhumacionExistente.codigo}). 
          No se puede registrar una nueva exhumación.`,
        );
      }
      ...
      const exhumacion = this.exhumacionRepository.create({
        ...
        inhumacion,
            ...
            const nuevaExhumacion = await this.exhumacionRepository.save(exhumacion);

      // Cambiar estado del nicho a DISPONIBLE y dejar observación
      nichoOriginal.estadoVenta = EstadoNicho.DISPONIBLE;
      const observacionBase = nichoOriginal.observaciones
        ? `${nichoOriginal.observaciones} | `
        : '';
      nichoOriginal.observaciones = `${observacionBase}Liberado por exhumación (${new Date().toLocaleDateString()})`;

      await this.nichoRepository.save(nichoOriginal);

      // Devolver resultado completo
      return {
        mensaje: 'Exhumación registrada y nicho liberado correctamente',
        exhumacion: nuevaExhumacion,
        nicho_actualizado: nichoOriginal,
      };
